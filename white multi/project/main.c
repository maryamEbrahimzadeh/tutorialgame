...
void multiplayer(void)
{
    ...
    }//agar naghshe khodesho khast bezaneh
    else read_map_original();
    ...
    while(1)
       {
           ...
           if(i==1)
           {
               ...
               gamer_cell.makanxx=makanx;
               gamer_cell.makanyy=makany;
               fwrite(&gamer_cell,sizeof(gamer_cell),1,fp);//selule man
           }
        ...
        struct cell_position mycell;
        mycell.x=makanx;
        mycell.y=makany;
        struct cell_position cellharif=cellnew(mycell);//farakhanie cellnew
            struct gamercell gamer_cell;
               gamer_cell.energy=0;
               gamer_cell.makanxx=cellharif.x;
               gamer_cell.makanyy=cellharif.y;
               fwrite(&gamer_cell,sizeof(gamer_cell),1,fp);//ta inja selul ha o dar file gharar dadm
   ...
   do
   {
        ...
        else
        {
                ...
                {
                    move();//ok
                    struct cell_move_data mn;
                    mn.cellenergy=current.energy;//current haman seluli hast ke dar  move moshakas mishavad bayad takan bokhorad
                    mn.move_direction=C;
                    struct cell_move_data U;
                    U=cell_move(mn);
                    cellharif=move2(cellharif.x,cellharif.y,U.move_direction);
                            struct gamercell gamer_cell2;
                            gemer_cell2.energy=U.cell_energy;
                            gamer_cell2.makanxx=cellharif.x;
                            gamer_cell2.makanyy=cellharif.y;
                            fseek(fp,1,SEEK_SET);
                            fwrite(&gamer_cell2,sizeof(gamer_cell),1,fp);break;
                ...
                if(B==2)//boost
                {
                    boost();//ok
                    struct cell_move_data mn;
                    mn.cellenergy=current.energy;//current haman seluli hast ke dar  boost moshakas mishavad bayad takan bokhorad
                    mn.move_direction=northsouth;//goftid chizi bezanim ke gheyre ona basheh.
                    struct cell_move_data U;
                    U=cell_move(mn);
                    struct gamercell gamer_cell2;
                            gemer_cell2.energy=U.cell_energy
                            gamer_cell2.makanxx=cellharif.x;//cellharif haman x va y ghabli ra negah dashteh ast.
                            gamer_cell2.makanyy=cellharif.y;
                            fseek(fp,1,SEEK_SET);
                            fwrite(&gamer_cell2,sizeof(gamer_cell),1,fp);break;
